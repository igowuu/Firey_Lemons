...
_METERS_PER_TICK = _METERS_PER_MOTOR_REV / FALCON_CPR
...
class SwerveModule:
    ...
    def getPosition(self) -> SwerveModulePosition:
        ticks = self.drive_motor.get_position().value
        meters = ticks * _METERS_PER_TICK
    ...
    def getState(self) -> SwerveModuleState:
        ...
        vel_ticks_per_100ms = self.drive_motor.get_velocity().value

        vel_ticks_per_sec = vel_ticks_per_100ms * 10.0

        meters_per_sec = vel_ticks_per_sec * _METERS_PER_TICK
        angle = self._read_cancoder_angle_radians()
        return SwerveModuleState(meters_per_sec, Rotation2d(angle))

    def _read_cancoder_angle_radians(self) -> float:
        maybe_deg = self.cancoder.get_absolute_position().value
        angle_rad = math.radians(maybe_deg)
