...
from constants import FeedForwardConstants, DriveConstants

_METERS_PER_MOTOR_REV = (2.0 * math.pi * DriveConstants.WHEEL_RADIUS) / DriveConstants.DRIVE_GEAR_RATIO
_MAX_VELOCITY = math.tau
_MAX_ACCELERATION = math.tau * 10
...
class SwerveModule:
    ...
    def __init__(self, name: str, drive_id: int, steer_id: int, cancoder_id: int) -> None:
        ...
        self.drive_pid = PIDController(
            FeedForwardConstants.DRIVE_kP, 
            FeedForwardConstants.DRIVE_kI, 
            FeedForwardConstants.DRIVE_kD
        )
        self.turning_pid = ProfiledPIDController(
            FeedForwardConstants.TURN_kP,
            FeedForwardConstants.TURN_kI,
            FeedForwardConstants.TURN_kD,
            wpimath.trajectory.TrapezoidProfile.Constraints(_MAX_VELOCITY, _MAX_ACCELERATION),
        )

        self.turning_pid.enableContinuousInput(-math.pi, math.pi)

        self.drive_ff = SimpleMotorFeedforwardMeters(
            FeedForwardConstants.DRIVE_kS, 
            FeedForwardConstants.DRIVE_kV, 
            FeedForwardConstants.DRIVE_kA
        )
        self.turn_ff = SimpleMotorFeedforwardMeters(
            FeedForwardConstants.TURN_kS, 
            FeedForwardConstants.TURN_kV, 
            FeedForwardConstants.TURN_kA
        )
